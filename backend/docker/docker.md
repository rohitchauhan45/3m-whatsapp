...
Need a database to back Prisma? Drop the snippet below into `docker-compose.yml` next to the app service:
...
Point the backend at it via:

```
DATABASE_URL="postgresql://boilerplate:password@localhost:5432/boilerplate_dev"
```

When you only need a throwaway instance (e.g., CI), run `npx prisma dev`, press `t` to print the TCP URL, and feed that directly to `DATABASE_URL`.
