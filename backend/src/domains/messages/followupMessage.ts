...
export const managerFollowUpFailureMessage = (
    managerName: string,
    failedCount: number,
    users: { name: string; number: string }[],
): string => {
    const lines: string[] = [];
    lines.push(`Hi ${managerName.trim()}`);
    lines.push("");
    lines.push(`⚠️ Follow-up failed for *${failedCount}* task${failedCount === 1 ? "" : "s"}.`);
    lines.push("");
    users.forEach((u, i) => {
        lines.push(`${i + 1}. ${u.name.trim()} (${u.number.trim()})`);
    });
    return lines.join("\n");
};
