...
const Layout: React.FC<LayoutProps> = ({ currentView, onChangeView, onLogout, children }) => {
  ...
  const NavItem = ({ view, icon: Icon, label }: { view: AppView; icon: any; label: string }) => (
    ...
    >
      <Icon size={20} className={`${currentView === view ? 'text-white' : 'text-gray-400 group-hover:text-gray-900'} transition-colors`} />
      <span>{label}</span>
    </button>
  );
  ...
  return (
    <div className="flex h-dvh overflow-hidden transition-colors duration-300">
      ...
      {/* Mobile Header */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white/80 backdrop-blur-xl border-b border-gray-200 z-40 flex items-center justify-between px-4">
      ...
      `}>
        ...
        <div className="p-6 flex items-center justify-between">
          <div className="flex items-center space-x-3">
             <div>
               <h1 className="font-bold text-lg leading-none tracking-tight text-gray-900">Admin Dashboard</h1>
             </div>
          </div>
        ...
        <div className="flex-1 px-4 py-2 space-y-1.5 overflow-y-auto no-scrollbar">
          <div className="px-4 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Learning</div>
          ...
          <div className="px-4 py-2 mt-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Resources</div>
        ...
        <div className="p-4 border-t border-gray-200 space-y-2 bg-gray-50/50">
          ...
          <button 
            onClick={onLogout}
            className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-red-500 hover:bg-red-50 hover:text-red-600 transition-colors"
          >
            <LogOut size={20} />
            <span className="font-medium">Sign Out</span>
          </button>

          <div className="mt-4 flex items-center p-3 rounded-xl bg-white border border-gray-200 shadow-sm">
             <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-gray-200 to-gray-300 border-2 border-white flex items-center justify-center text-xs font-bold text-gray-700">LS</div>
             <div className="ml-3 overflow-hidden">
               <p className="text-sm font-semibold text-gray-900 truncate">Law Student</p>
